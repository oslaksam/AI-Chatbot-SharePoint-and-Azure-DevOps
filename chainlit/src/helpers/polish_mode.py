...
def load_handbook_pl(filepath: str):
    ...
    try:
        ...
        docs = polish_mode_splitter.split_documents(documents)
...
class PolandHandbookChatMode(ChatMode):
    async def setup(self):
        msg = cl.Message(content=f"Processing `TEMPLATE Poland employee handbook`...", disable_feedback=True)
        await msg.send()
        ...
        poland_collection_name = "TEMPLATE_poland_handbook"
        ...
        if count == 0:
            ...
            if not docs:
                await cl.Message(content="Error occurred while processing the file.", author="Error").send()
                return
    ...
    async def on_chat_resume(self, thread):
        print("Resuming polish mode chat")
