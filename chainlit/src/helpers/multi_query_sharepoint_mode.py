...
class SharePointMultiQueryChatMode(ChatMode):
    async def setup(self):
        ...
        run_model = configure_model(settings)
        multi_query_chain = LLMChain(llm=run_model, prompt=multi_query_prompt, output_parser=LineListOutputParser())
    ...
    async def change_settings(self, settings):
        ...
        chain = AggregatingConversationalRetrievalChain.from_llm(
            ...
            response_if_no_docs_found=chain.response_if_no_docs_found,
            search_with_original_question=chain.search_with_original_question,
            multi_query_chain=chain.multi_query_chain,
        )
        cl.user_session.set("chain", chain)
    ...
    async def on_chat_resume(self, thread):
        print("Resuming multiquery sharepoint chat")
